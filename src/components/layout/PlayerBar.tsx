...
export function PlayerBar() {
  ...
  return (
    <>
      ...
      {currentSet && (
        <div className="bg-surface-raised border-t border-border shrink-0">
          ...
          <div className="flex items-center h-16 sm:h-20 px-3 sm:px-4 gap-2 sm:gap-4">
            {/* Set info */}
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 w-[120px] sm:w-[240px]">
            ...
            <div className="flex-1 flex flex-col items-center gap-0.5 sm:gap-1 max-w-[600px]">
              <div className="flex items-center gap-3 sm:gap-4">
                <button onClick={playPrevious} className="hidden sm:block text-text-secondary hover:text-text-primary transition-colors">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z" /></svg>
                </button>
                <button onClick={togglePlay} className="w-9 h-9 sm:w-8 sm:h-8 bg-text-primary rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-transform">
                  {isPlaying
                    ? <svg className="w-4 h-4 text-surface" fill="currentColor" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" /></svg>
                    : <svg className="w-4 h-4 text-surface ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>}
                </button>
                <button onClick={playNext} className="hidden sm:block text-text-secondary hover:text-text-primary transition-colors">
            ...
            <div className="hidden sm:flex items-center gap-2 w-[140px] justify-end">
              ...
              <input type="range" min={0} max={1} step={0.01} value={isMuted ? 0 : volume} onChange={(e) => setVolume(parseFloat(e.target.value))} className="w-20 h-1 bg-border rounded-full appearance-none cursor-pointer accent-accent" />
            </div>
          </div>
        </div>
      )}
