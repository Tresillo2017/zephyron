...
function UserActionsMenu({
  ...
  const item = (label: string, onClick: () => void, danger = false, disabled = false) => (
    <button
      ...
      onMouseEnter={(e) => {
        if (!disabled && !loading)
          e.currentTarget.style.background = danger ? 'hsl(0, 60%, 50% / 0.12)' : 'hsl(var(--b3) / 0.6)'
      }}
  ...
  const divider = <div key="div" className="h-px my-1" style={{ background: 'hsl(var(--b4) / 0.4)' }} />
  ...
  return (
    <>
      <button
        ...
        onMouseEnter={(e) => { e.currentTarget.style.background = 'hsl(var(--b3) / 0.6)'; e.currentTarget.style.color = 'hsl(var(--c1))' }}
...
export function UsersTab() {
  ...
  const loadUsers = async (currentPage = page, currentSearch = search) => {
    ...
    try {
      ...
      const res = await authClient.admin.listUsers({ query: query as Parameters<typeof authClient.admin.listUsers>[0]['query'] })
      if (res.data) {
  ...
  const handleSetRole = async (user: User) => {
    ...
    await withLoading(user.id, async () => {
      const res = await authClient.admin.setRole({ userId: user.id, role: newRole as 'admin' | 'user' })
      if (res.error) { showToast(res.error.message ?? 'Failed to update role', false); return }
  ...
  const handleBanConfirm = async () => {
    ...
    setModal(null)
    await withLoading(user.id, async () => {
      const res = await authClient.admin.banUser(opts as Parameters<typeof authClient.admin.banUser>[0])
      if (res.error) { showToast(res.error.message ?? 'Failed to ban user', false); return }
      setUsers((prev) => prev.map((u) => u.id === user.id ? { ...u, banned: true, banReason: banReason.trim() || 'Banned by admin' } : u))
  ...
  const handleUnban = async (user: User) => {
    await withLoading(user.id, async () => {
      ...
      if (res.error) { showToast(res.error.message ?? 'Failed to unban', false); return }
  ...
  const handleRevokeConfirm = async () => {
    ...
    await withLoading(user.id, async () => {
      ...
      if (res.error) { showToast(res.error.message ?? 'Failed to revoke sessions', false); return }
  ...
  const handleImpersonate = async (user: User) => {
    await withLoading(user.id, async () => {
      ...
      if (res.error) { showToast(res.error.message ?? 'Failed to impersonate', false); return }
  ...
  const handleDeleteConfirm = async () => {
    ...
    await withLoading(user.id, async () => {
      ...
      if (res.error) { showToast(res.error.message ?? 'Failed to delete user', false); return }
  ...
  const handleSetPassword = async (user: User, newPassword: string) => {
    await withLoading(user.id, async () => {
      ...
      if (res.error) { showToast(res.error.message ?? 'Failed to set password', false); return }
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <p className="text-sm ml-auto shrink-0" style={{ color: 'hsl(var(--c3))' }}>
      ...
      {toast && (
        <div
          ...
          style={{
            background: toast.ok ? 'hsl(var(--h3) / 0.12)' : 'hsl(0, 60%, 50% / 0.12)',
            color: toast.ok ? 'hsl(var(--h3))' : 'hsl(0, 60%, 65%)',
            boxShadow: `inset 0 0 0 1px ${toast.ok ? 'hsl(var(--h3) / 0.25)' : 'hsl(0, 60%, 50% / 0.25)'}`,
          }}
        >
          {toast.msg}
        </div>
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-center py-8" style={{ color: 'hsl(var(--c3))' }}>
        ...
        <div className="card !p-0 overflow-hidden">
          ...
          {users.map((user, index) => (
            <div
              ...
              style={{
                borderTop: index > 0 ? '1px solid hsl(var(--b4) / 0.2)' : 'none',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = 'hsl(var(--b4) / 0.15)' }}
              onMouseLeave={(e) => { e.currentTarget.style.background = '' }}
            >
              {/* User info */}
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {user.banned && user.banReason && (
                    ...
                    >
                      {user.banReason.length > 24 ? user.banReason.slice(0, 24) + '…' : user.banReason}
                    </span>
                  )}
                </div>
                <p className="text-xs truncate" style={{ color: 'hsl(var(--c3))' }}>{user.email}</p>
                <p className="text-[10px]" style={{ color: 'hsl(var(--c3))' }}>{formatRelativeTime(user.createdAt)}</p>
              </div>
              ...
              <div className="w-14 text-center">
                <span className="text-sm tabular-nums" style={{ color: 'hsl(var(--c2))' }}>
              ...
              <div className="w-20 text-center">
                {user.banned ? (
                  ...
                  <span className="text-xs" style={{ color: 'hsl(140, 55%, 60%)' }}>Active</span>
                )}
              </div>
      ...
      <Modal
        ...
        onClose={() => { setModal(null); setBanReason(''); setBanExpiry('') }}
        title={`Ban ${modal?.type === 'ban' ? modal.user.name : ''}` }
      >
        <div className="flex flex-col gap-4">
          <p className="text-sm" style={{ color: 'hsl(var(--c2))' }}>
            Banned users cannot sign in. You can set an optional reason and expiry.
          </p>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-[var(--font-weight-medium)]" style={{ color: 'hsl(var(--c2))' }}>
              Duration
            </label>
            <div className="flex flex-wrap gap-2">
              {BAN_EXPIRY_OPTIONS.map((opt) => (
                <button
                  ...
                  style={{
                    background: banExpiry === opt.value ? 'hsl(var(--h3) / 0.15)' : 'hsl(var(--b4) / 0.4)',
                    color: banExpiry === opt.value ? 'hsl(var(--h3))' : 'hsl(var(--c2))',
                    boxShadow: banExpiry === opt.value ? 'inset 0 0 0 1px hsl(var(--h3) / 0.4)' : 'none',
                  }}
                >
                  {opt.label}
                </button>
          ...
          <div className="flex gap-2 justify-end pt-1">
            <Button variant="ghost" onClick={() => { setModal(null); setBanReason(''); setBanExpiry('') }}>
              Cancel
            </Button>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button variant="danger" onClick={handleRevokeConfirm}>Revoke Sessions</Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal
        ...
        onClose={() => { setModal(null); setNewPassword('') }}
        title={`Set Password — ${modal?.type === 'set-password' ? modal.user.name : ''}`}
      >
        <div className="flex flex-col gap-4">
          <p className="text-sm" style={{ color: 'hsl(var(--c2))' }}>
          ...
          />
          <div className="flex gap-2 justify-end">
            <Button variant="ghost" onClick={() => { setModal(null); setNewPassword('') }}>
              Cancel
            </Button>
            <Button
              variant="primary"
